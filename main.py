...
def totalize_hh_results(city_id, popular_program_languages):
    ...
    for program_language in popular_program_languages:
        ...
        program_languages_statistic[program_language] = {
...
def create_job_table(table_values, vacancy_source):
    heading_row = [["Язык программирование", "Вакансий найдено",
                    "Вакансий обработано", "Средняя зарплата"]]
    for language, salary_of_language in table_values.items():
        iter_language = [language]
        iter_language.extend(salary_of_language.values())
...
def main():
    popular_program_languages = ["Python", "Java",
                                 "Javascript", "Golang", "C++", "php"]
    load_dotenv()
    city_hh_id = get_hh_area_id(os.getenv("CITY"))
    city_sj_id = get_sj_area_id(os.getenv("CITY"))
    print(create_job_table(totalize_sj_results
                           (city_sj_id, popular_program_languages),
                           f"SuperJob {os.getenv('CITY')}"))
    print(create_job_table(totalize_hh_results
                           (city_hh_id,
                            popular_program_languages),
                           f"HeadHunter {os.getenv('CITY')}"))
